...
def generate_csv_active():
    ...
    SELECT
        ...
        templatesDB.CDT(CAST(ALMACEN AS UNSIGNED))                                     as facility_code,
    ...
    FROM (
        SELECT
            'GPOSAN' AS company_code,
            templatesDB.CDT(CAST(INV.ALMACEN AS UNSIGNED)) AS facility_code,
...
def generate_damage_csv():
    ...
    # Guardar los DataFrames en un archivo CSV
    with open('RDM_ISS_InboundShipments_24A.csv', 'w', newline='') as f:
        header_df.to_csv(f, index=False)
        detail_df.to_csv(f, index=False)
